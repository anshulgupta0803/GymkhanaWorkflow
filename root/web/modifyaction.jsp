        ...
        <%
            ...
            if(type == 1)
            {
                if((id_type != null) && (id_type.equals("club")))
                {
                    ...
                    Integer Club_ID = Integer.parseInt(request.getParameter("club_id"));
                        ...
                        try
                        {
                            if((Obj.isalpha(Club_Name)) && (Obj.isalpha(Coordinator)) && (Obj.isalpha(Faculty_Mentor)) && (Obj.isalpha(Council)) && (new Double(Club_Budget_Allocated) > 0))
                            {
                                if((new Date(Club_Date_Activation)).compareTo(new Date(Club_Date_Valid_Upto)) <= 0)
                                {
                                    ...
                                    if(DbObj.new_modify_club(Club_ID, Club_Name, Coordinator, Faculty_Mentor, Council, new_Club_Date_Activation, new_Club_Date_Valid_Upto, new Double(Club_Budget_Allocated), Club_Description, Club_Created_By))
                                    {
                                       //out.print("");
                                       out.print("<script>alert(\"Changes were sucessfully done !!\");window.location.assign(\"index.jsp\");</script>");
                                       //response.sendRedirect("index.jsp");
                                    }
                                    else out.print("<script>alert(\"Unable to modify the club.\");window.location.assign(\"index.jsp\");</script>");
                                }
                                else
                                {
                                    out.print("<script>alert(\"Unable to modify the club.\");window.location.assign(\"index.jsp\");</script>");
                                }
            ...
            else if (type == 0)
            {
                    ...
                    if((event_name != null) && (event_club_id != null ) && (event_organizer != null) && (event_start != null) && (event_end != null))
                    {
                        ...
                        if((Obj.isalpha(event_organizer)) && (event_club_id > 0))
                        {
                            if((new Date(event_start)).compareTo(new Date(event_end)) <= 0)
                            {
                                ...
                                UpdateDb DbObj = new UpdateDb();
                                if(DbObj.new_modify_event(event_id, event_name, event_club_id, new_event_start, new_event_end, event_details, event_organizer, event_created_by))
                                {
                                   //out.print("");
                                   out.print("<script>alert(\"Changes were sucessfully done !!\");window.location.assign(\"index.jsp\");</script>");
                                   //response.sendRedirect("index.jsp");
                                }
                                else out.print("<script>alert(\"Unable to modify the event.\");window.location.assign(\"index.jsp\");</script>");
                            }
                            else
                            {
                                out.print("<script>alert(\"Unable to modify the event.\");window.location.assign(\"index.jsp\");</script>");
                            }
                        }
                        else
                            {
                    ...
                    if((venue_name != null) && (venue_event_id != null ) && (venue_entry != null) && (venue_exit != null))
                    {
                        ...
                        if((Obj.isalphanum(venue_name)) && (venue_event_id>0))
                        {
                            if((new Date(venue_entry)).compareTo(new Date(venue_exit)) <= 0)
                            {
                                ...
                                UpdateDb DbObj = new UpdateDb();
                                if(DbObj.new_modify_venue (venue_id, venue_name, venue_event_id, new_venue_entry , new_venue_exit, venue_created_by))
                                {
                                   //out.print("");
                                   out.print("<script>alert(\"Changes were sucessfully done !!\");window.location.assign(\"index.jsp\");</script>");
                                   //response.sendRedirect("index.jsp");
                                }
                                else out.print("<script>alert(\"Unable to modify the venue.\");window.location.assign(\"index.jsp\");</script>");
                            }
                            else
                            {
                                out.print("<script>alert(\"Unable to modify the venue.\");window.location.assign(\"index.jsp\");</script>");
                            }
                        }
                        else
                            {
                ...
                if((id_type != null) && (id_type.equals("budget")))
                {
                    ...
                    Integer budget_id = Integer.parseInt (request.getParameter("budget_id"));
                    ...
                    if((budget_event_id != null) && (budget_demanded != null))
                    {
                        ...
                        if((Obj.isnum(budget_event_id)) && (new Double(budget_demanded) > 0))
                        {
                                ...
                                UpdateDb DbObj = new UpdateDb();
                                if(DbObj.new_modify_budget(budget_id, new Double(budget_demanded),new Integer(budget_event_id),budget_details, budget_created_by,new Timestamp(10)))
                                {
                                   //out.print("");
                                   out.print("<script>alert(\"Changes were sucessfully done !!\");window.location.assign(\"index.jsp\");</script>");
                                   //response.sendRedirect("index.jsp");
                                }
                                else out.print("<script>alert(\"Unable to modify the budget.\");window.location.assign(\"index.jsp\");</script>");
                        }
                        else
                            {
                                out.print("<script>alert(\"Unable to modify the budget.\");window.location.assign(\"index.jsp\");</script>");
                            }
                    }
                    else
                            {
